"""고등 2,000단어 IPA·한글발음·품사 메타 자동 생성.

수기 검수된 Day 01(HIGH_PRON 40개)는 그대로 유지하고, 나머지는 자동 생성.
- IPA: eng-to-ipa
- 한글 발음: CMUdict ARPAbet → 단어장식 한글
- 품사: 뜻 어미 규칙

결과: data/high_book_meta.json
"""
...
VOCA_PATH = ROOT / "voca_high.txt"
OUT_PATH = ROOT / "data" / "high_book_meta.json"
...
def extract_manual_overrides(script_path: Path) -> tuple[dict[str, tuple[str, str]], dict[str, str]]:
    ...
    ns: dict = {}
    m = re.search(r"^HIGH_PRON = (\{.*?\n\})", src, re.M | re.S)
    if not m:
        raise ValueError("HIGH_PRON 블록을 찾지 못했습니다.")
    exec(f"HIGH_PRON = {m.group(1)}", ns)
    # 고등은 수기 품사뜻 블록이 없으면 어미 규칙만 사용
    pos: dict[str, str] = {}
    m_pos = re.search(r"^HIGH_POS_MEANINGS = (\{.*?\n\})", src, re.M | re.S)
    if m_pos:
        exec(f"HIGH_POS_MEANINGS = {m_pos.group(1)}", ns)
        pos = ns["HIGH_POS_MEANINGS"]
    return ns["HIGH_PRON"], pos
